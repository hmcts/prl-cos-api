package uk.gov.hmcts.reform.prl.filter.requestbodylogger;
 ...
 *
 * <p>This bean is only enabled if the property 'prl.request-body-logger' is set to true in the application properties
 * file.
 */
@Component
@ConditionalOnProperty(prefix = "prl.request-body-logger", name = "enabled", havingValue = "true")
@Slf4j
...
public class HttpRequestBodyLoggerFilter implements Filter {
    ...
    private void logRequestBody(String contextPath, String requestBody) {
        ...
        try {
            ...
            log.info("{} request body (non-JSON):\n{}", contextPath, requestBody);
        }
    }
}
