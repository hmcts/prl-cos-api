...
import static uk.gov.hmcts.reform.prl.config.templates.Templates.PRL_LET_ENG_C100_AP8;
import static uk.gov.hmcts.reform.prl.config.templates.Templates.PRL_LET_ENG_C100_RE5;
import static uk.gov.hmcts.reform.prl.config.templates.Templates.PRL_LET_ENG_C100_RE6;
import static uk.gov.hmcts.reform.prl.config.templates.Templates.PRL_LET_ENG_FL401_AP1;
import static uk.gov.hmcts.reform.prl.config.templates.Templates.PRL_LET_ENG_FL401_AP2;
...
public class ServiceOfApplicationService {
    ...
    private void sendNotificationCaNonPersonalApplicantCitizen(CaseData caseData, String authorization,
        ...
        if (ContactPreferences.email.equals(selectedApplicant.getValue().getContactPreferences())) {
            ...
            fieldsMap.put(COVER_LETTER_TEMPLATE, Templates.PRL_LET_ENG_C100_AP6);
            ...
            sendPostWithAccessCodeLetterToParty(
                ...
                bulkPrintDetails,
                selectedApplicant,
                generateCoverLetterBasedOnCaseAccess(authorization, caseData, selectedApplicant, Templates.PRL_LET_ENG_C100_AP6),
    ...
    private String handlePersonalServiceNotificationsDaCitizenOptions(CaseData caseData, String authorization,
        ...
        if (SoaCitizenServingRespondentsEnum.unrepresentedApplicant
            ...
            if (ContactPreferences.email.equals(caseData.getApplicantsFL401().getContactPreferences())) {
                Map<String, String> fieldsMap = new HashMap<>();
                fieldsMap.put(AUTHORIZATION, authorization);
                fieldsMap.put(COVER_LETTER_TEMPLATE, PRL_LET_ENG_FL401_AP2);
                sendEmailToCitizenLipPersonalServiceCaDa(
                    caseData,
                    emailNotificationDetails,
                    applicant,
    ...
    private String handleNotificationsDaPersonalApplicantLip(CaseData caseData, String authorization,
        ...
        packFdocs.addAll(getRe2OrRe3WithoutAccessCode(caseData, authorization, respondent));
        ...
        if (ContactPreferences.email.equals(caseData.getApplicantsFL401().getContactPreferences())) {
            Map<String, String> fieldsMap = new HashMap<>();
            fieldsMap.put(AUTHORIZATION, authorization);
            fieldsMap.put(COVER_LETTER_TEMPLATE, PRL_LET_ENG_FL401_AP1);
            sendEmailToCitizenLipPersonalServiceCaDa(
                caseData,
                emailNotificationDetails,
                applicant,
    ...
    private String handlePersonalServiceNotificationsCaSolicitorOptions(CaseData caseData, String authorization,
        ...
        if (SoaSolicitorServingRespondentsEnum.applicantLegalRepresentative.equals(caseData.getServiceOfApplication()
            ...
            whoIsResponsibleForServing = caseData.getApplicants().getFirst().getValue().getRepresentativeFullName();
            ...
            caseData.getRespondents().forEach(respondent -> packHiDocs.addAll(generateAccessCodeLetter(authorization,
                                                                                                    caseData,
                                                                                                    respondent,
                                                                                                    null,
            ...
            dynamicData.put("name", caseData.getApplicants().getFirst().getValue().getRepresentativeFullName());
            dynamicData.put("c100", true);
            dynamicData.put(DASH_BOARD_LINK, manageCaseUrl + PrlAppsConstants.URL_STRING + caseData.getId());
            populateLanguageMap(caseData, dynamicData);
            EmailNotificationDetails emailNotification = serviceOfApplicationEmailService
                .sendEmailUsingTemplateWithAttachments(
                    authorization, caseData.getApplicants().getFirst().getValue().getSolicitorEmail(),
                    packHiDocs,
    ...
    private void handleNotificationsCaSolicitorPersonalCourtAdminBailiff(CaseData caseData, String authorization,
        ...
        EmailNotificationDetails emailNotification = serviceOfApplicationEmailService.sendEmailUsingTemplateWithAttachments(authorization,
                                                   caseData.getApplicants().getFirst().getValue().getSolicitorEmail(),
                                                   packjDocs,
    ...
    private List<Document> getDocumentsForCaorBailiffToServeRespondents(CaseData caseData, String authorization,
        ...
        for (Element<PartyDetails> respondent: caseData.getRespondents()) {
            re5Letters.addAll(generateCoverLetterBasedOnCaseAccess(authorization, caseData, respondent,
    ...
    private List<Document> getCoverLettersAndRespondentPacksForDaApplicantSolicitor(CaseData caseData, String authorization,
        ...
        if (attachLetters) {
            Element<PartyDetails> respondent = Element.<PartyDetails>builder()
                ...
                .value(caseData.getRespondentsFL401())
                .build();
            reLetters.addAll(getRe2OrRe3WithoutAccessCode(caseData, authorization, respondent));
        }
    ...
    private List<Document> getRe2OrRe3WithoutAccessCode(CaseData caseData, String authorization,
                                                  Element<PartyDetails> respondent) {
        if (CaseUtils.isApplyOrderWithoutGivingNoticeToRespondent(caseData)) {
            return generateAccessCodeLetter(authorization, caseData, respondent, null, PRL_LET_ENG_FL401_RE2);
        } else {
            return generateAccessCodeLetter(authorization, caseData, respondent, null, PRL_LET_ENG_FL401_RE3);
        }
    }
    ...
    private void sendNotificationsDaNonPersonalRespondent(CaseData caseData, String authorization,
        ...
        if (CollectionUtils.isNotEmpty(respondentFl401)) {
            String emailAddress = respondentFl401.getFirst().getValue().getSolicitorEmail();
            String servedParty = respondentFl401.getFirst().getValue().getLabelForDynamicList();
            List<Document> docs = new ArrayList<>();
            ...
            if (CaseUtils.hasLegalRepresentation(respondentFl401.getFirst().getValue())) {
                log.info("Legal rep present for respondent");
                servedParty = respondentFl401.getFirst().getValue().getRepresentativeFullName();
            } else {
                log.info("respondent is unrepresented");
                coverLetter = getRe1OrRe4BasedOnWithOrWithoutNotice(caseData, authorization, respondentFl401.getFirst(),
                                                                    CaseUtils.getCaseInvite(respondentFl401.getFirst().getId(),
            ...
            if (sendEmail) {
                ...
                sendPostWithAccessCodeLetterToParty(caseData,
                                                    ...
                                                    bulkPrintDetails, respondentFl401.getFirst(),
    ...
    private void sendNotificationsDaNonPersonalApplicant(CaseData caseData, String authorization,
        ...
        if (CollectionUtils.isNotEmpty(applicantFl401)) {
            String emailAddress = applicantFl401.getFirst().getValue().getEmail();
            String servedParty = applicantFl401.getFirst().getValue().getLabelForDynamicList();
            List<Document> docs = new ArrayList<>();
            Map<String, Object> dynamicData = EmailUtils.getCommonSendgridDynamicTemplateData(caseData);
            ...
            if (CaseUtils.hasLegalRepresentation(applicantFl401.getFirst().getValue())) {
                emailAddress = applicantFl401.getFirst().getValue().getSolicitorEmail();
                servedParty = applicantFl401.getFirst().getValue().getRepresentativeFullName();
            } else {
                ...
                coverLetters = generateCoverLetterBasedOnCaseAccess(
                    authorization,
                    caseData, applicantFl401.getFirst(), PRL_LET_ENG_FL401_AP2);
                docs.addAll(coverLetters);
                if (!ContactPreferences.email.equals(applicantFl401.getFirst().getValue().getContactPreferences())) {
                    sendEmail = false;
                }
            }
            List<Document> packDocs = getNotificationPack(caseData, PrlAppsConstants.A, staticDocs);
            ...
            if (sendEmail) {
                ...
                sendPostWithAccessCodeLetterToParty(caseData,
                                                    authorization, packDocs,
                                                    bulkPrintDetails, applicantFl401.getFirst(),
        ...
        selectedRespondents.forEach(respondent -> {
            if (CaseUtils.hasLegalRepresentation(respondent.getValue())) {
                ...
                if (respondent.getValue().getAddress() != null && StringUtils.isNotEmpty(respondent.getValue().getAddress().getAddressLine1())) {
                    ...
                    List<Document> coverLetters = generateCoverLetterBasedOnCaseAccess(authorization, caseData, respondent, PRL_LET_ENG_C100_RE5);
        ...
        caseData.getApplicants().forEach(selectedApplicant -> {
            if (!CaseUtils.hasLegalRepresentation(selectedApplicant.getValue())) {
                if (ContactPreferences.email.equals(selectedApplicant.getValue().getContactPreferences())) {
                    Map<String, String> fieldsMap = new HashMap<>();
                    fieldsMap.put(AUTHORIZATION, authorization);
                    fieldsMap.put(COVER_LETTER_TEMPLATE, PRL_LET_ENG_C100_AP7);
        ...
        caseData.getApplicants().forEach(selectedApplicant -> {
            if (!CaseUtils.hasLegalRepresentation(selectedApplicant.getValue())) {
                ...
                if (ContactPreferences.email.equals(selectedApplicant.getValue().getContactPreferences())) {
                    Map<String, String> fieldsMap = new HashMap<>();
                    fieldsMap.put(AUTHORIZATION, authorization);
                    fieldsMap.put(TEMPLATE, PRL_LET_ENG_C100_AP7);
                    ...
                    if (CaseCreatedBy.CITIZEN.equals(caseData.getCaseCreatedBy()) && CaseUtils.isCitizenAccessEnabled(selectedApplicant.getValue())) {
                        coverLetters = generateAccessCodeLetter(
                            authorization,
                            caseData,
                            selectedApplicant,
                            null,
                        ...
                        coverLetters = CaseUtils.getCoverLettersForParty(
        ...
        caseData.getApplicants().forEach(applicant -> {
            if (!CaseUtils.hasLegalRepresentation(applicant.getValue())) {
                if (ContactPreferences.email.equals(applicant.getValue().getContactPreferences())) {
                    ...
                    Map<String, String> fieldsMap = new HashMap<>();
                    fieldsMap.put(AUTHORIZATION, authorization);
                    fieldsMap.put(COVER_LETTER_TEMPLATE, PRL_LET_ENG_C100_AP8);
                    sendEmailToCitizenLipPersonalServiceCaDa(
                        caseData,
                        emailNotificationDetails,
                        applicant,
        ...
        caseData.getApplicants().forEach(applicant -> {
            if (!CaseUtils.hasLegalRepresentation(applicant.getValue())) {
                if (ContactPreferences.email.equals(applicant.getValue().getContactPreferences())) {
                    ...
                    Map<String, String> fieldsMap = new HashMap<>();
                    fieldsMap.put(AUTHORIZATION, authorization);
                    fieldsMap.put(TEMPLATE, PRL_LET_ENG_C100_AP8);
                    ...
                    if (CaseCreatedBy.CITIZEN.equals(caseData.getCaseCreatedBy()) && CaseUtils.isCitizenAccessEnabled(applicant.getValue())) {
                        ...
                        coverLetters = CaseUtils.getCoverLettersForParty(applicant.getId(),
                                                          caseData.getServiceOfApplication()
    ...
    private void sendEmailToCitizenLipByCheckingDashboardAccess(CaseData caseData,
        ...
        if (CaseCreatedBy.CITIZEN.equals(caseData.getCaseCreatedBy()) && CaseUtils.isCitizenAccessEnabled(party.getValue())
            ...
            coverLetters.addAll(generateAccessCodeLetter(fieldMap.get(AUTHORIZATION), caseData, party, null, fieldMap.get(TEMPLATE)));
        } else {
    ...
    public Map<String, Object> handleAboutToSubmit(CallbackRequest callbackRequest, String authorisation) {
        ...
        if (caseData.getServiceOfApplication() != null && SoaCitizenServingRespondentsEnum.unrepresentedApplicant
            ...
            if (C100_CASE_TYPE.equalsIgnoreCase(CaseUtils.getCaseTypeOfApplication(caseData))) {
                if (ObjectUtils.isNotEmpty(caseData.getApplicants().getFirst().getValue().getResponse())) {
                    if (ObjectUtils.isNotEmpty(caseData.getApplicants().getFirst().getValue().getResponse().getCitizenFlags())) {
                        caseData.getApplicants().getFirst().getValue().getResponse().getCitizenFlags().setIsApplicationToBeServed(YesOrNo.Yes);
                    } else {
                        caseData.getApplicants().getFirst().getValue().getResponse()
                            .setCitizenFlags(CitizenFlags.builder().isApplicationToBeServed(Yes).build());
                    }
                } else {
                    caseData.getApplicants().getFirst().getValue().setResponse(Response.builder()
    ...
    private void publishHearingDate(CallbackRequest callbackRequest, String authorisation, Map<String, Object> caseDataMap) {
        ...
        if (null != hearings && CollectionUtils.isNotEmpty(hearings.getCaseHearings())
            && LISTED.equals(hearings.getCaseHearings().getFirst().getHmcStatus()) && null != hearings.getCaseHearings().get(
            0).getNextHearingDate()) {
            caseDataMap.put(
                WA_PRODUCT_HEARING_BUNDLE_ON,
                DAYS.between(
                                LocalDate.now(),
                                hearings.getCaseHearings().getFirst().getNextHearingDate().toLocalDate()) > 3
                                ? hearings.getCaseHearings().getFirst().getNextHearingDate().minusDays(3).format(
        ...
        selectedApplicants.forEach(applicant -> {
            if (CaseUtils.hasLegalRepresentation(applicant.getValue())) {
                ...
                if (ContactPreferences.email.equals(applicant.getValue().getContactPreferences())) {
                    Map<String, String> fieldsMap = new HashMap<>();
                    fieldsMap.put(AUTHORIZATION, authorization);
                    fieldsMap.put(TEMPLATE, Templates.PRL_LET_ENG_C100_AP6);
                    sendEmailToCitizenLipByCheckingDashboardAccess(caseData,
                                                             emailNotificationDetails,
                                                                   applicant,
                                                             packDocs,
                    ...
                    List<Document> coverLetters;
                    if (CaseCreatedBy.CITIZEN.equals(caseData.getCaseCreatedBy()) && CaseUtils.isCitizenAccessEnabled(applicant.getValue())) {
                        coverLetters = generateAccessCodeLetter(authorization, caseData, applicant, null, Templates.PRL_LET_ENG_C100_AP6);
                    } else {
                        coverLetters = CaseUtils.getCoverLettersForParty(applicant.getId(),
                                                                        caseData.getServiceOfApplication()
                                                                            .getUnServedApplicantPack()
    ...
    private EmailNotificationDetails sendEmailCaPersonalApplicantLegalRep(CaseData caseData, String authorization,
        ...
        if (caseData.getApplicants().getFirst().getValue().getSolicitorEmail() != null) {
            try {
                log.info(
    ...
    private Document getCoverLetterDocument(String authorisation, String template, Map<String, Object> dataMap) {
        ...
        return Document.builder()
            .documentUrl(accessCodeLetter.getUrl())
            .documentFileName(accessCodeLetter.getDocName())
            .documentBinaryUrl(accessCodeLetter.getBinaryUrl())
    ...
    public Map<String, Object> populateAccessCodeMap(CaseData caseData, Element<PartyDetails> party, CaseInvite caseInvite) {
        ...
        if (C100_CASE_TYPE.equals(CaseUtils.getCaseTypeOfApplication(caseData))) {
            dataMap.put("applicantName", caseData.getApplicants().getFirst().getValue().getLabelForDynamicList());
        }
    ...
    private void buildPacksConfidentialCheckC100Personal(String authorization,
        ...
        if (SoaSolicitorServingRespondentsEnum.courtAdmin
            ...
            caseData.getRespondents().forEach(respondent -> {
                List<Document> coverLetters = new ArrayList<>();
                coverLetters.addAll(generateAccessCodeLetter(
                    authorization,
            ...
            final SoaPack unservedRespondentPack = SoaPack.builder()
                ...
                .partyIds(wrapElements(caseData.getApplicants().getFirst().getId().toString()))
    ...
    private void generateUnServedPacksForCourtAdminBailiff(String authorization,
        ...
        List<Document> packjDocs = generatePartiesPackDocsWithCoverLetter(authorization, caseData, caseData.getApplicants(),
                                                           PRL_LET_ENG_C100_AP8, PrlAppsConstants.J, c100StaticDocs,
        ...
        final SoaPack unServedApplicantPack = SoaPack.builder()
            ...
            .partyIds(isCitizen
                          ...
                          : wrapElements(caseData.getApplicants().getFirst().getId().toString()))
            .servedBy(PRL_COURT_ADMIN)
            .personalServiceBy(serviceBy)
            .packCreatedDate(CaseUtils.getCurrentDate())
    ...
    private List<Document> generatePartiesPackDocsWithCoverLetter(String authorization,
        ...
        parties.forEach(party -> {
            if (!CaseUtils.hasLegalRepresentation(party.getValue())) {
                List<Document> coverLetters = new ArrayList<>(generateCoverLetterBasedOnCaseAccess(
                    authorization,
                    caseData,
                    party,
                    template
    ...
    private SoaPack generatePacksForApplicantLipC100Personal(String authorization, CaseData caseData,
        ...
        caseData.getApplicants().forEach(applicant -> {
            if (!CaseUtils.hasLegalRepresentation(applicant.getValue())) {
                List<Document> coverLetters = new ArrayList<>(generateCoverLetterBasedOnCaseAccess(authorization,
                                                                                                   caseData,
                                                                                                   applicant,
                                                                                                   PRL_LET_ENG_C100_AP7
                ));
    ...
    public List<Document> generateCoverLetterBasedOnCaseAccess(String authorization,
                                                               CaseData caseData,
                                                               Element<PartyDetails> party,
                                                               String template) {
    ...
    private Map<String, Object> getPacksForConfidentialCheckDaNonPersonalService(String authorization, CaseData caseData,
        ...
        if (CollectionUtils.isNotEmpty(applicantFl401)) {
            List<Document> docs = new ArrayList<>();
            List<Element<CoverLetterMap>> coverLetterMap = new ArrayList<>();
            String partyId = String.valueOf(applicantFl401.getFirst().getValue().getSolicitorPartyId());
            if (!CaseUtils.hasLegalRepresentation(applicantFl401.getFirst().getValue())) {
                log.info("applicant lip");
                partyId = String.valueOf(applicantFl401.getFirst().getId());
                List<Document> coverletters = generateCoverLetterBasedOnCaseAccess(authorization, caseData, applicantFl401.getFirst(),
                                                                                   PRL_LET_ENG_FL401_AP2);
                docs.addAll(coverletters);
        ...
        if (CollectionUtils.isNotEmpty(respondentFl401)) {
            String partyId = String.valueOf(respondentFl401.getFirst().getValue().getSolicitorPartyId());
            List<Document> docs = new ArrayList<>();
            List<Element<CoverLetterMap>> coverLetterMap = new ArrayList<>();
            if (!CaseUtils.hasLegalRepresentation(respondentFl401.getFirst().getValue())) {
                log.info("respondent lip");
                partyId = String.valueOf(respondentFl401.getFirst().getId());
                List<Document> coverLetter = getRe1OrRe4BasedOnWithOrWithoutNotice(caseData, authorization, respondentFl401.getFirst(),
                                                                                   CaseUtils.getCaseInvite(respondentFl401.getFirst().getId(),
                                                                                                           caseData.getCaseInvites()));
                docs.addAll(coverLetter);
                coverLetterMap.add(element(UUID.fromString(partyId), CoverLetterMap.builder()
    ...
    private void generatePacksForConfidentialCheckDaApplicantLip(String authorization, CaseData caseData,
                                                                 ...
                                                                 List<Document> fl401StaticDocs) {
        Element<PartyDetails> applicant = element(caseData.getApplicantsFL401().getPartyId(), caseData.getApplicantsFL401());
        List<Document> applicantCoverLetters = generateCoverLetterBasedOnCaseAccess(authorization, caseData, applicant,
        ...
        if (!CaseUtils.hasLegalRepresentation(caseData.getApplicantsFL401())) {
            List<Document> coverLetters = generateCoverLetterBasedOnCaseAccess(authorization, caseData,
                                                 element(caseData.getApplicantsFL401().getPartyId(),
                                                         caseData.getApplicantsFL401()), PRL_LET_ENG_FL401_AP2);
    ...
    private Map<String, Object>  getPacksForConfidentialCheckDaApplicantSolicitor(String authorization, CaseData caseData,
        ...
        Element<PartyDetails> respondent = Element.<PartyDetails>builder()
            ...
            .value(caseData.getRespondentsFL401())
            .build();
        List<Document> reLetters = new ArrayList<>(getRe2OrRe3WithoutAccessCode(caseData, authorization, respondent));
        List<Element<CoverLetterMap>> coverletterMap = new ArrayList<>();
    ...
    private void buildUnservedRespondentPackC100NonPersonal(String authorization, Map<String, Object> caseDataUpdated, CaseData caseData,
        ...
        selectedPartyIds.forEach(partyId -> {
            ...
            if (party.isPresent() && !CaseUtils.hasLegalRepresentation(party.get().getValue())) {
                List<Document> coverLetters = new ArrayList<>();
                coverLetters.addAll(generateCoverLetterBasedOnCaseAccess(authorization, caseData, party.get(), PRL_LET_ENG_C100_RE5));
        ...
        for (String partyId : selectedPartyIds) {
            ...
            if (party.isPresent() && !CaseUtils.hasLegalRepresentation(party.get().getValue())) {
                List<Document> coverLetters = new ArrayList<>(generateCoverLetterBasedOnCaseAccess(
                    ...
                    caseData,
                    party.get(),
                    Templates.PRL_LET_ENG_C100_AP6
                ));
    ...
    public CaseData sendNotificationsAfterConfidentialCheckSuccessful(CaseData caseData, String authorization) {
        ...
        if (ObjectUtils.isNotEmpty(unservedCymruPack) && CollectionUtils.isNotEmpty(unservedCymruPack.getPartyIds())) {
            emailNotificationDetails.addAll(sendEmailToCafcassInCase(
                caseData,
                unservedCymruPack.getPartyIds().getFirst().getValue(),
            ...
            try {
                EmailNotificationDetails emailNotification = serviceOfApplicationEmailService
                    .sendEmailNotificationToLocalAuthority(
                        authorization,
                        caseData,
                        unServedLaPack.getPartyIds().getFirst().getValue(),
        ...
        if (isAutoLinkRequired(eventId, caseDataMap)
            ...
            applicants.stream()
                ...
                .ifPresent(party -> {
                    ...
                    if (CollectionUtils.isNotEmpty(caseData.getUserInfo()) && isNotEmpty(caseData.getUserInfo().getFirst().getValue())) {
                        user = user.toBuilder()
                            .idamId(caseData.getUserInfo().getFirst().getValue().getIdamId())
                            .email(caseData.getUserInfo().getFirst().getValue().getEmailAddress())
                            .build();
                    } else {
    ...
    private boolean isPartyEmailSameAsIdamEmail(CaseData caseData,
        ...
        return CollectionUtils.isNotEmpty(caseData.getUserInfo())
            ...
            caseData.getUserInfo().getFirst().getValue().getEmailAddress());
    }
}
