...
public class TaskListRenderer {
    ...
    private static List<TaskSection> generateTaskListV2(CaseData caseData, Map<Event, Task> tasks) {
        ...
        final TaskSection peopleInTheCase = newSection(ADD_PEOPLE_TO_THE_CASE)
                .withInfo("<p>If children live with another party in the case (other than the applicant or respondent) you can add"
                + " these details to 'Other people in the case.' if you do complete this section, you must keep it up to date.<p>"
                + "<p>Os oes plant yn byw gyda’r parti arall yn yr achos (heblaw’r ceisydd neu’r atebydd), gallwch ychwanegu’r manylion "
                + "hyn yn y blwch ‘Pobl eraill yn yr achos.’ Os ydych chi’n cwblhau’r adran hon, mae’n rhaid i chi gadw’r wybodaeth yn gyfredol.<p>")
