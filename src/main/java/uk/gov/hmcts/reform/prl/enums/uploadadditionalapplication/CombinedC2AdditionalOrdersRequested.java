...
public enum CombinedC2AdditionalOrdersRequested {
    ...
    @CCD(label = "C2 - Request an order relating to a child - Prohibited steps order")
    ...
    @CCD(label = "C2 - Request an order relating to a child - Specific issue order")
    @JsonProperty("C2_SPECIFIC_ISSUE_ORDER")
    C2_SPECIFIC_ISSUE_ORDER(
        "C2_SPECIFIC_ISSUE_ORDER",
        "C2 - Request an order relating to a child - Specific issue order"
    ),
    @CCD(label = "C2 - Ask to submit evidence the court has not requested")
    ...
    @CCD(label = "C2 - Ask to share documents with someone else")
    ...
    @CCD(label = "C2 - Ask the court to appoint an expert")
    @JsonProperty("C2_REQUEST_TO_APPOINT_AN_EXPERT")
    C2_REQUEST_TO_APPOINT_AN_EXPERT(
        "C2_REQUEST_TO_APPOINT_AN_EXPERT",
        "C2 - Ask the court to appoint an expert"
    ),
    @CCD(label = "C2 - Get permission for an application if the court previously stopped you")
