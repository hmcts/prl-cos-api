...
public class CafcassServiceUtil {
    ...
    public static CaseData getCaseDataWithUploadedDocs(String caseId, String fileName, String typeOfDocument,
        ...
        if (caseData.getCafcassUploadedDocs() != null) {
            uploadedDocumentsList = caseData.getDocumentManagementDetails().getCafcassQuarantineDocsList();
            uploadedDocumentsList.add(quarantineLegalDoc);
        } else {
            uploadedDocumentsList = new ArrayList<>();
            uploadedDocumentsList.add(quarantineLegalDoc);
        }
        return CaseData.builder().id(Long.parseLong(caseId)).documentManagementDetails(DocumentManagementDetails.builder().cafcassQuarantineDocsList(
            uploadedDocumentsList).build()).build();
    }
