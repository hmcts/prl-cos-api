...
import uk.gov.hmcts.reform.prl.models.complextypes.ApplicantChildRelations;
...
public class CallbackController {
    ...
    public AboutToStartOrSubmitCallbackResponse prePopulateApplicantToChildRelation(
        ...
        Map<String, Object> caseDataUpdated = callbackRequest.getCaseDetails().getData();
        List<Element<ApplicantChildRelations>> applicantChildRelationsList = new ArrayList();
        CaseData caseData = CaseUtils.getCaseData(callbackRequest.getCaseDetails(), objectMapper);
        caseData.getApplicants().forEach(eachApplicant ->
                caseData.getChildren().forEach(eachChild -> {
                    ApplicantChildRelations applicantChildRelations =  ApplicantChildRelations.builder()
                            .applicantFirstName(eachApplicant.getValue().getFirstName())
                            .applicantLastName(eachApplicant.getValue().getLastName()).childName(
                                    String.format(PrlAppsConstants.FORMAT, eachChild.getValue().getFirstName(),
                                            eachChild.getValue().getLastName())).build();
                    applicantChildRelationsList.add(Element.<ApplicantChildRelations>builder().value(applicantChildRelations).build());
                })
        );
        caseDataUpdated.put("applicantRelationsToChild", applicantChildRelationsList);
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "Bad Request", content = @Content)})
    @SecurityRequirement(name = "Bearer Authentication")
    public AboutToStartOrSubmitCallbackResponse deleteApplicantToChildRelation(
            @RequestHeader(HttpHeaders.AUTHORIZATION) @Parameter(hidden = true) String authorisation,
            @RequestBody uk.gov.hmcts.reform.ccd.client.model.CallbackRequest callbackRequest
    ) {
        CaseData caseData = CaseUtils.getCaseData(callbackRequest.getCaseDetails(), objectMapper);
        ...
        Map<String, Object> caseDataUpdated = callbackRequest.getCaseDetails().getData();
        caseDataUpdated.put("applicantRelationsToChild", null);
        caseDataUpdated.put("originalApplicantRelationsToChild", caseData.getApplicantRelationsToChild());
        log.info("data {}",new Gson().toJson(caseDataUpdated));
        return AboutToStartOrSubmitCallbackResponse.builder().data(caseDataUpdated).build();
    }
}
