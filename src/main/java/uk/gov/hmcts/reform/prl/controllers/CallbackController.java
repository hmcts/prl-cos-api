...
public class CallbackController {
    ...
    @ApiResponses(value = {
        ...
        @ApiResponse(responseCode = "400", description = "Bad Request", content = @Content)})
    @SecurityRequirement(name = "Bearer Authentication")
    public AboutToStartOrSubmitCallbackResponse populatePartyInformation(@RequestHeader(HttpHeaders.AUTHORIZATION)
    ...
    @PostMapping(path = "/check-casename-for-html-injection", consumes = APPLICATION_JSON, produces = APPLICATION_JSON)
    @Operation(description = "check the casename for html injection")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Callback processed.",
            content = @Content(mediaType = "application/json", schema = @Schema(implementation = AboutToStartOrSubmitCallbackResponse.class))),
        @ApiResponse(responseCode = "400", description = "Bad Request", content = @Content)})
    @SecurityRequirement(name = "Bearer Authentication")
    public AboutToStartOrSubmitCallbackResponse checkCasenameForHtmlInjection(
        @RequestHeader(HttpHeaders.AUTHORIZATION) @Parameter(hidden = true) String authorisation,
        @RequestHeader(PrlAppsConstants.SERVICE_AUTHORIZATION_HEADER) String s2sToken,
        @RequestBody CallbackRequest callbackRequest
    ) {
        if (authorisationService.isAuthorized(authorisation, s2sToken)) {
            Map<String, Object> caseDataUpdated = callbackRequest.getCaseDetails().getData();
            if ((null != caseDataUpdated.get(APPLICANT_OR_RESPONDENT_CASE_NAME) && checkStringForHtmlTags(caseDataUpdated
                .get(APPLICANT_OR_RESPONDENT_CASE_NAME).toString()))
                || (null != caseDataUpdated.get(APPLICANT_CASE_NAME) && checkStringForHtmlTags(caseDataUpdated
                .get(APPLICANT_CASE_NAME).toString()))) {
                return AboutToStartOrSubmitCallbackResponse
                    .builder()
                    .errors(List.of("You have entered an invalid character. Special characters <,>,{,} are not allowed."))
                    .build();
            }
    ...
    private Boolean checkStringForHtmlTags(String stringToBeChecked) {
