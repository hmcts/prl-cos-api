...
public class CustomOrderService {
     ...
     */
    private void copyStyles(XWPFDocument sourceDoc, XWPFDocument targetDoc) {
        try {
            ...
            for (String styleId : usedStyleIds) {
                ...
                if (sourceStyle != null && targetStyles.getStyle(styleId) == null) {
                    // Copy via the underlying CT object
     ...
     */
    private void copyNumberingDefinitions(XWPFDocument sourceDoc, XWPFDocument targetDoc) {
        try {
            ...
            // Get or create numbering in target document
            XWPFNumbering targetNumbering = targetDoc.getNumbering();
            if (targetNumbering == null) {
                targetNumbering = targetDoc.createNumbering();
            }

            // Copy abstract numbering definitions by adding them to target
            // Use addAbstractNum which returns the new abstract num ID
            ...
            for (int i = 0; i < 50; i++) {
                try {
                    var abstractNum = sourceNumbering.getAbstractNum(java.math.BigInteger.valueOf(i));
                    if (abstractNum != null) {
     ...
     */
    private void copyImages(XWPFDocument sourceDoc, XWPFDocument targetDoc) {
        try {
     ...
     */
    private void copyFootnotes(XWPFDocument sourceDoc, XWPFDocument targetDoc) {
        try {
