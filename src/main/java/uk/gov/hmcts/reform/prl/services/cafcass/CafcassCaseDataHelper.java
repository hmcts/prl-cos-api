...
public class CafcassCaseDataHelper {
    ...
    public boolean hasCafcassCaseDataChanged(CaseDetails caseDetails, CaseDetails caseDetailsBefore) {
        ...
        return !Objects.equals(
            normaliseForComparison(prepareForComparison(caseDetails, authorisation)),
            normaliseForComparison(prepareForComparison(caseDetailsBefore, authorisation))
        );
    }

    private Map<String, Object> normaliseForComparison(CafCassCaseDetail cafCassCaseDetail) {
        ...
        if (caseData instanceof Map<?, ?> caseDataMap) {
            removeElementIds(caseDataMap, "childAndApplicantRelations");
            removeElementIds(caseDataMap, "childAndRespondentRelations");
            removeElementIds(caseDataMap, "childAndOtherPeopleRelations");
        }
    ...
    private void removeElementIds(Map<?, ?> caseDataMap, String fieldName) {
        ...
        if (collection instanceof List<?> elements) {
            elements.stream()
                ...
                .forEach(element -> element.put("id", null));
        }
    }
