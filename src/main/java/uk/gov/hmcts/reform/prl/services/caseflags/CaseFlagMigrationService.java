...
public class CaseFlagMigrationService {
    ...
    public Map<String, Object> migrateCaseForCaseFlags(Map<String, Object> caseDataMap) {
        ...
        Map<String, Object> caseFlagsMap = objectMapper.convertValue(caseData.getAllPartyFlags(), Map.class);
        return caseFlagsMap;
    }
    ...
    private void addToExternalOrInternalFlags(String partyType, Element<FlagDetail> flag, CaseData caseData, boolean isExternal) {
        switch (partyType) {
            case DA_APPLICANT -> setCaseFlags(flag, caseData.getAllPartyFlags().getDaApplicantInternalFlags(),
                                              caseData.getAllPartyFlags().getDaApplicantExternalFlags(), isExternal
            );
            case DA_RESPONDENT -> setCaseFlags(flag, caseData.getAllPartyFlags().getDaRespondentInternalFlags(),
                                               caseData.getAllPartyFlags().getDaRespondentExternalFlags(), isExternal
            );
            case CA_APPLICANT_1 -> setCaseFlags(flag, caseData.getAllPartyFlags().getCaApplicant1InternalFlags(),
                                                caseData.getAllPartyFlags().getCaApplicant1ExternalFlags(), isExternal
            );
            case CA_APPLICANT_2 -> setCaseFlags(flag, caseData.getAllPartyFlags().getCaApplicant2InternalFlags(),
                                                caseData.getAllPartyFlags().getCaApplicant2ExternalFlags(), isExternal
            );
            case CA_APPLICANT_3 -> setCaseFlags(flag, caseData.getAllPartyFlags().getCaApplicant3InternalFlags(),
                                                caseData.getAllPartyFlags().getCaApplicant3ExternalFlags(), isExternal
            );
            case CA_APPLICANT_4 -> setCaseFlags(flag, caseData.getAllPartyFlags().getCaApplicant4InternalFlags(),
                                                caseData.getAllPartyFlags().getCaApplicant4ExternalFlags(), isExternal
            );
            case CA_APPLICANT_5 -> setCaseFlags(flag, caseData.getAllPartyFlags().getCaApplicant5InternalFlags(),
                                                caseData.getAllPartyFlags().getCaApplicant5ExternalFlags(), isExternal
            );
            case CA_RESPONDENT_1 -> setCaseFlags(flag, caseData.getAllPartyFlags().getCaRespondent1InternalFlags(),
                                                 caseData.getAllPartyFlags().getCaRespondent1ExternalFlags(), isExternal
            );
            case CA_RESPONDENT_2 -> setCaseFlags(flag, caseData.getAllPartyFlags().getCaRespondent2InternalFlags(),
                                                 caseData.getAllPartyFlags().getCaRespondent2ExternalFlags(), isExternal
            );
            case CA_RESPONDENT_3 -> setCaseFlags(flag, caseData.getAllPartyFlags().getCaRespondent3InternalFlags(),
                                                 caseData.getAllPartyFlags().getCaRespondent3ExternalFlags(), isExternal
            );
            case CA_RESPONDENT_4 -> setCaseFlags(flag, caseData.getAllPartyFlags().getCaRespondent4InternalFlags(),
                                                 caseData.getAllPartyFlags().getCaRespondent4ExternalFlags(), isExternal
            );
            case CA_RESPONDENT_5 -> setCaseFlags(flag, caseData.getAllPartyFlags().getCaRespondent5InternalFlags(),
    ...
    private void setCaseFlags(Element<FlagDetail> flag, Flags internalFlags,Flags externalFlags, boolean isExternal) {
