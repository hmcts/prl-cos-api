...
import static uk.gov.hmcts.reform.prl.enums.State.SUBMITTED_NOT_PAID;
...
class HwfProcessUpdateCaseStateServiceTest {
    ...
    void givenCaseWithNoHwfNumber_whenCheck_thenNoUpdate() {
        ...
        SearchResult searchResult = SearchResult.builder()
            .total(1)
            .cases(List.of(caseDetails(1234764578342789L, SUBMITTED_NOT_PAID, caseData)))
            .build();
        mockSearchCases(searchResult);
    ...
    void givenMultiplePageResults_whenCheck_thenRetrievesAllCases() {
        CaseData caseData = caseData(null, null, null);
        CaseDetails caseDetails = caseDetails(1234764578342789L, SUBMITTED_NOT_PAID, caseData);
    ...
    void givenCaseWithHwfNumberAndNotPaid_whenCheck_thenNoUpdates() {
        CaseData caseData = caseData("HWF-12345", "2025-1750002799132", "2025-12-25");
    ...
    @ParameterizedTest
    @MethodSource("givenCaseWithHwfNumberAndPaid_whenCheck_thenHandles")
    void givenCaseWithHwfNumberAndPaid_whenCheck_thenHandles(State currentState, State expectedUpdatedState) {
        CaseData caseData = caseData("HWF-12345", "2025-1750002799132", "2025-12-25");

        SearchResult searchResult = SearchResult.builder()
            .total(1)
            .cases(List.of(caseDetails(1234764578342789L, currentState, caseData)))
            .build();
        mockSearchCases(searchResult);
    ...
    private static Stream<Arguments> givenCaseWithHwfNumberAndPaid_whenCheck_thenHandles() {
        return Stream.of(State.values())
            .map(state -> {
                State expected = (state == State.CASE_WITHDRAWN || state == SUBMITTED_NOT_PAID)
    ...
    void givenMultipleCases_whenCheck_thenHandlesAllCases() {
        CaseData caseData1 = caseData("HWF-12345", "ref1", "2025-12-25");
        CaseData caseData2 = caseData("HWF-12345", "ref2", null);

        SearchResult searchResult = SearchResult.builder()
            .total(2)
            .cases(List.of(
                caseDetails(1234764578342789L, SUBMITTED_NOT_PAID, caseData1),
                caseDetails(1234764578342790L, SUBMITTED_NOT_PAID, caseData2))
            )
            .build();
        mockSearchCases(searchResult);
    ...
    private CaseDetails caseDetails(long caseId, State state, CaseData caseData) {
